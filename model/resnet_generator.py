...
class ResnetGenerator128(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        num_w = 128 + 10  # control obj features for MaskRegressNet. 14 if rot is label_encoded.
    ...
    def forward(self, z, bbox, z_im=None, y=None): # rm rot
        ...
        # print('rot shape ', rotation.shape)  # [50, 16]
        # rotation = rotation.view(-1, 1)
        # rotation = rotation.view(b*o, -1)
        # r = rotation.size(1)
        # print('r', r)
        z = z.view(b * o, -1)  # [800, 128]
        label_embedding = label_embedding.view(b * o, -1)  # [512, 180];  with 10 emb. [512, 10]; bs=50 [800, 10]
        # print('lab embded: ', label_embedding.shape)

        # [32, 16, 308] with 10 emb.[32, 16, 138]; [50, 16, 138]
        latent_vector = torch.cat((z, label_embedding), dim=1).view(b, o, -1)
        # print('latent vec: ', latent_vector.shape)

        w = self.mapping(latent_vector.view(b * o, -1))  # [512, 308] with 10 emb. [512, 138]
        # print('w: ', w.shape)
        # preprocess bbox
        bbox = self.mask_regress(w, bbox)  # out: [32, 16, 64, 64]
